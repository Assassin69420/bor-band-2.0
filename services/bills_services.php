...
function get_bill_userplanid(string|null $related_plan_id, string|null $related_service_id, mysqli $db)
{
	...
	if ($related_plan_id !== "") {
		...
		$sql = "SELECT bill_id, user_id, amount, due_date, paid_date, related_plan, cgst_percentage, sgst_percentage, service_name, U.name as username, U.id as account_id FROM bills B
						INNER JOIN useraccount U ON U.id = B.user_id
